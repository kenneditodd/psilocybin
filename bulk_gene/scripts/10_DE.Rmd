...
meta <- dge.filtered.norm$samples[,c("sample_id","group2")]
...
# extract DEG comparison table
degs <- read.delim2(file = paste0(out, model,"DEG_tables/DEG_comparison_FDRq_0.10_LFC_0.20.tsv"))
rownames(degs) <- degs$direction
degs$direction <- NULL
comparisons <- colnames(degs)

powerTable <- as.data.frame(matrix(NA, nrow = 1, ncol = length(comparisons)))
colnames(powerTable) <- comparisons

for (i in comparisons) {
  # mean read counts in reference group
  print(i)
  refGroup <- str_match(i, ".+_vs_(.+)")[,2]
  refMean <- mean(mean.group[,refGroup])

  # loop through comparisons and check power
  pow <- check.power(
              nGenes = maxGenes,                  # total num genes
              pi0 = degs["NotSig", i] / maxGenes, # nonsig proportion
              m = 5,                              # sample size, n=10 per group
              mu = refMean,                       # mean read counts in a control group
              disp = myDisp,                      # dispersion for all genes
              fc = 0.2,                           # fold change for DE genes
              up = degs["Up", i] / maxGenes,      # upreg proportion
...
for (i in comparisons) {
  ...
  pow <- check.power(
              nGenes = maxGenes,                  # total num genes
              pi0 = degs["NotSig", i] / maxGenes, # nonsig proportion
              m = 10,                             # sample size, n=10 per group
              mu = refMean,                       # mean read counts in a control group
              disp = myDisp,                      # dispersion for all genes
              fc = 0.2,                           # fold change for DE genes
              up = degs["Up", i] / maxGenes,      # upreg proportion
