...
```{r set_variables}
out <- "../../results/high_hbb_removed/"
out2 <- "high_hbb_removed"
model <- "DEGs/"
...
for (dim in dims) {
  ...
  for (numGenes in topGenes) {
    ...
    toplot <- data.frame(DimX = mds$x, 
                         DimY = mds$y,
                         Sample = rownames(mds$distance.matrix.squared),
                         RNA_extraction = v$targets$RNA_extraction_batch)
    ...
    p <- ggplot(toplot, aes(x = DimX, y = DimY, color = RNA_extraction, label = Sample)) + 
      geom_point(size = 3) +
      theme_bw() +
      geom_text(aes(label = Sample), size=4, vjust = -1) +
      xlab(x.lab) + ylab(y.lab) + 
