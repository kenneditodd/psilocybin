...
```{r}
up <- h.24h.up[h.24h.up %in% kl.24h.up]
up <- up[up %in% kh.24h.up]
up <- up[!up %in% l.24h.up]
write.table(x = as.data.frame(up),
            file = paste0(out,model,"upset/24_hour_comparison_24_genes_shared_up.tsv"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)

up <- l.24h.up[l.24h.up %in% kl.24h.up]
up <- up[up %in% kh.24h.up]
up <- up[!up %in% h.24h.up]
up <- up[!up %in% k.24h.up]
write.table(x = as.data.frame(up),
            file = paste0(out,model,"upset/24_hour_comparison_11_genes_shared_up.tsv"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)

up <- l.24h.up[l.24h.up %in% h.24h.up]
up <- up[up %in% kl.24h.up]
up <- up[up %in% kh.24h.up]
up <- up[!up %in% k.24h.up]
write.table(x = as.data.frame(up),
            file = paste0(out,model,"upset/24_hour_comparison_1_gene_shared_up.tsv"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)

down <- l.24h.down[l.24h.down %in% kl.24h.down]
down <- down[down %in% kh.24h.down]
down <- down[!down %in% h.24h.down]
write.table(x = as.data.frame(up),
            file = paste0(out,model,"upset/24_hour_comparison_23_genes_shared_down.tsv"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)

down <- l.24h.down[l.24h.down %in% h.24h.down]
down <- down[down %in% kl.24h.down]
down <- down[down %in% kh.24h.down]
down <- down[!down %in% k.24h.down]
write.table(x = as.data.frame(up),
            file = paste0(out,model,"upset/24_hour_comparison_10_genes_shared_down.tsv"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)
```
