...
meta$group2 <- factor(paste0(meta$group, ".", meta$sex))
...
```{r remove_high_hemoglobin}
# Add Hbb-bs to metadata
hbb <- as.data.frame(cpm(dge, log = TRUE))["Hbb-bs",]
dge$samples$hbb <- as.numeric(hbb)
...
# Option 2 filtering method
# all samples in a group (sal/psi) must have CPM > 1
# create separate df
sal.samples <- dge$samples$sample_id[dge$samples$treatment %in% "saline"]
num.sal <- length(sal.samples)
psi.samples <- dge$samples$sample_id[dge$samples$treatment %in% "psilocybin"]
num.psi <- length(psi.samples)
# true when CPM >= 1
sal.df <- cpm[,colnames(cpm) %in% sal.samples] >= 1
psi.df <- cpm[,colnames(cpm) %in% psi.samples] >= 1
# find row sum
sal.df <- as.data.frame(sal.df)
psi.df <- as.data.frame(psi.df)
sal.df$rsum <- as.numeric(Matrix::rowSums(sal.df))
psi.df$rsum <- as.numeric(Matrix::rowSums(psi.df))
# keep if all samples in group had CPM >= 1
sal.df$keep <- sal.df$rsum == num.sal
psi.df$keep <- psi.df$rsum == num.psi
# keep gene if TRUE in sal or psi df
keep <- sal.df$keep | psi.df$keep
dge.filtered <- dge[keep,,keep.lib.sizes = FALSE]


# save
mean.group.final <- mean.group[keep,]
...
remove(meta,mean.group,cpm,keep,start,end,mean.group.final,psi.df,sal.df)
```
...
# Power analysis
- power - usually set to 0.8 which means 80% probability
-
```{r}
