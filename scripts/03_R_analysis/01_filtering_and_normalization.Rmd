...
meta <- meta %>% ungroup() %>% arrange(group)
...
hbb_log2cpm <- as.data.frame(cpm(dge, log = TRUE))["Hbb-bs",]
dge$samples$hbb_log2cpm <- as.numeric(hbb_log2cpm)
remove(hbb_log2cpm)
...
for (dim in dims) {
  ...
  for (numGenes in topGenes) {
    ...
    toplot <- data.frame(DimX = mds$x, 
                         DimY = mds$y,
                         Sample = rownames(mds$distance.matrix.squared),
                         extraction = dge$samples$RNA_extraction_batch)
    ...
    p <- ggplot(toplot, aes(x = DimX, y = DimY, color = extraction, label = Sample)) + 
      geom_point(size = 3) +
