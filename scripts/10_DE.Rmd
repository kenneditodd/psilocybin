...
## Gene network and top 5 mod
- The degree of a node is a measure of connectivity:
  - In an undirected graph, the degree is the number of edges (connections) a node has.
  - In a signed network, the adjacency matrix weights are continuous, and igraph calculates the degree as the sum of edge weights for a node (weighted degree). \
```{r}
# required libraries
library(igraph)
library(ggraph)
...
# define unique colors (excluding grey)
...
# init varibles
top_hubs_per_module <- list()  # List to store top hubs for each module

# loop through each module color
for (module_color in module_colors) {
  # genes in the current module
  ...
  # Check if there are any edges left after applying the threshold
  if (sum(module_adjacency != 0) == 0) {
    # If no edges, skip this module and continue to the next one
    message("Skipping ", module_color, ": Network has no edges.")
    next
  }
  ...
  # If the network is not connected, skip the current iteration and move to the next module
  if (!is_connected(network)) {
    message("Skipping ", module_color, ": Network is not connected.")
    next  # Skip to the next module if the network is not connected
  }
  
  # save top 5 genes, calculate degree (hub score) for each gene
  node_degrees <- degree(network)
  top_5_genes <- names(sort(node_degrees, decreasing = TRUE)[1:5])
  top_hubs_per_module[[module_color]] <- top_5_genes
  ...
  g <- ggraph(tidy_network, layout = "kk") +
    # edges
    ...
    # theme and title
    theme_void() +
    labs(title = paste0(module_color, " network (threshold = ", threshold, ")"),
         color = "degree",
         size = "degree")
...
top_hubs_df <- data.frame(
  Module = rep(names(top_hubs_per_module), each = 5),
  Top_Hub_Genes = unlist(top_hubs_per_module)
)
top_hubs_df$Module_Order <- rownames(top_hubs_df)
top_hubs_df <- top_hubs_df[,c(1,3,2)]

# save the top 5 hubs
write.table(
  top_hubs_df,
  file = paste0(wgcna_out, "gene_network_corr_", threshold, "/", 
                "top_5_hub_genes_per_module.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
