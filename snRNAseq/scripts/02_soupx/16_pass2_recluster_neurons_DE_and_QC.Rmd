...
neurons_annotated <- readRDS(paste0(
  "../../rObjects/", filtering_method, "_recluster_neurons_pass2_annotated_seurat_obj.rds"))
```

## UMAP
```{r annotated_umap}
neurons_annotated$annotated_clusters <- neurons_annotated$azimuth_subclass_filtered
cluster_colors <- get_cluster_colors(
  num_clusters = length(unique(neurons_annotated$azimuth_subclass_filtered))
)
...
```{r}
path <- paste0(out, "UMAP/annotated_UMAP.pdf")
...
# UMAP percent_mt
f1 <- FeaturePlot(neurons_annotated, 
            reduction = "umap", 
            features = "percent_mt")  + 
...
# UMAP nCount
f2 <- FeaturePlot(neurons_annotated, 
            reduction = "umap",
            features = "nCount_RNA") + 
  scale_colour_gradientn(colours = c("blue","lightblue","yellow","orange","red"))
f2
...
# UMAP nFeature
f3 <- FeaturePlot(neurons_annotated, 
            reduction = "umap", 
...
# UMAP percent_ribo
f4 <- FeaturePlot(neurons_annotated, 
            reduction = "umap", 
...
f5 <- FeaturePlot(neurons_annotated, 
            reduction = "umap", 
...
# UMAP percent_hb
f6 <- FeaturePlot(neurons_annotated, 
            reduction = "umap", 
            features = "percent_hb") + 
  scale_colour_gradientn(colours = c("blue","lightblue","yellow","orange","red"))
f6
...
```{r percent_cells}
# timepoint
b1 <- neurons_annotated@meta.data %>%
...
# sex
b2 <- neurons_annotated@meta.data %>%
...
# sample
b3 <- neurons_annotated@meta.data %>%
...
# phase
b4 <- neurons_annotated@meta.data %>%
...
# mito.factor
b5 <- neurons_annotated@meta.data %>%
...
# group
b6 <- neurons_annotated@meta.data %>%
...
# group2
b7 <- neurons_annotated@meta.data %>%
...
# dose
b8 <- neurons_annotated@meta.data %>%
...
## Cells per sample
```{r cells_per_sample}
data <- as.data.frame(table(neurons_annotated$sample_id))
...
## Cells per cluster
```{r cells_per_cluster}
data <- as.data.frame(table(neurons_annotated$annotated_clusters))
...
ncells <- ggplot(data, aes(x = cluster, y = frequency, fill = cluster)) + 
  ...
  geom_text(aes(label = frequency), 
            position=position_dodge(width=0.9), 
            vjust=-0.25) +
  scale_y_continuous(breaks = seq(0,70000, by = 10000), limits = c(0,70000)) +
...
# extract data
data <- data.frame(cluster = neurons_annotated$annotated_clusters,
                   sample = neurons_annotated$sample_id)
data_summary <- data %>%
...
remove(data_summary,data,ncells)
```

## Sex check
```{r sex_check}
dot <- DotPlot(neurons_annotated,
               features = c("Uty","Xist"),
               group.by = "sample_id")
dot
```
...
for (i in 1:length(files)) {
  ...
  for (j in cell_types) {
    ...
    # file name
    prefix2 <- paste0(out, "DEGs_MAST_pct_", pct_thresh, "/metascape_input/", p.comparison,
                     "/", p.comparison, "_", gsub(" ", "_", tolower(j_replace)), 
...
makeShinyApp(obj = shiny.obj, 
             scConf = sc.config, 
             gene.mapping = TRUE,
             shiny.title = "E. coli Mice Pass 3")
