...
mouse_annotated <- readRDS("../../rObjects/pass4_downsampled_annotated_subtypes_seurat_obj.rds")
```
...
# Protein coding only
Filtering DEG tables to protein coding only.
Also removing ENSMUSG00000095041 because it's likely not meaningful and skewing volcanoes.
```{r compare_DEGs}
# get file paths
files <- list.files(paste0(out, "DEGs_MAST_pct_0.6/DEG_tables"))
...
for (i in seq_along(files)) {
  ...
  # read table
  df <- read.table(paste0(out, "DEGs_MAST_pct_0.6/DEG_tables/", files[i]),
                   sep = "\t", 
                   header = TRUE)
  df <- df %>% rename(gene_name = gene)
  
  # add gene meta
  df <- left_join(x = df, y = genes, by = "gene_name")
  
  # filter by protein_coding
  df <- df[df$gene_type == "protein_coding",]
  df <- df[!df$gene_id == "ENSMUSG00000095041",]
  
  # save
  write.table(x = df, 
              file = paste0(out, "DEGs_MAST_pct_0.6_protein_only/DEG_tables/", 
                            files[i]), 
              sep = "\t", 
              quote = FALSE, 
              row.names = FALSE)
...
## Cells per sample
```{r cells_per_sample}
data <- as.data.frame(table(mouse_annotated$sample_id))
...
data <- data.frame(cluster = mouse_annotated$annotated_clusters,
                   sample = mouse_annotated$sample_id)
data_summary <- data %>%
...
# get file paths
files <- list.files(paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/DEG_tables"))
files <- files[grepl(".+_DEGs.tsv", files)]
...
for (i in files) {
  ...
  # read table
  df <- read.table(paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/DEG_tables/", i),
                   sep = "\t", 
                   header = TRUE)
...
# add any cell types missing (0 DEGs)
for (i in cell_types) {
  if (!i %in% colnames(deg.df)) {
    deg.df[[i]] <- 0
  }
}
...
# reorder columns based on levels
deg.df <- deg.df[,cell_types]

# save
prefix <- paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/DEG_comparison/",
    ...
    if(num != 0) {
      ...
      p <-
        ...
        geom_text_repel(data = up30,
                        aes(x = avg_log2FC, y= -log10(p_val), label = gene_name), 
                        color = "maroon", 
                        fontface="italic",
      ...
      prefix <- paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/volcano/", p.comparison)
...
# get file paths
files <- list.files(paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/DEG_tables"), full.names = TRUE)
files <- files[grepl(".+_DEGs.tsv", files)]
...
for (i in 1:length(files)) {
  ...
  prefix <- paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/metascape_input/", 
                   p.comparison)
  if (!dir.exists(prefix)) {
    dir.create(prefix, recursive = TRUE)
  }
  ...
  for (j in cell_types) {
    ...
    # file name
    prefix <- paste0(out, "DEGs_MAST_pct_", pct_thresh, "_protein_only/metascape_input/", 
                     p.comparison, "/", p.comparison, "_", gsub(" ", "_", tolower(j)), 
